...
print("\n" + "=" * 120)
print(f"  {'Model':<16}" + "".join(f"{c:>14}" for c in COLS))
print("  " + "-" * 117)
for name, res in RESULTS.items():
    row = f"  {name:<16}"
    for c in COLS:
        v    = res[c]
        row += f"{str(round(v, 3)):>14}"
    print(row)
print("=" * 120)
...
st.markdown("*Institute of Liver and Biliary Diseases (ILBS) &middot; GBM Model &middot; AUC 0.854 &middot; Sensitivity 0.84*")
st.divider()
...
with st.sidebar:
    ...
    # For notebook execution, directly use the file path
    uploaded = "/content/NVCB_anonymised_dataset.xlsx"
    # In a live Streamlit app, you would use: uploaded = st.file_uploader("Upload NVCB_anonymised_dataset.xlsx", type=["xlsx"])
    st.divider()
...
# The st.stop() call is no longer needed since 'uploaded' is directly set
# if uploaded is None:
...
if predict_clicked:
    ...
    # Build full feature vector — non-input features set to 0
