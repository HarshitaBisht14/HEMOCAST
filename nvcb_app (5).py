# -*- coding: utf-8 -*-
"""
NVCB Prediction Tool — Streamlit App
Non-Variceal Coagulopathic Bleeding in Acute Decompensated Cirrhosis
Institute of Liver and Biliary Diseases (ILBS)
GBM Model | AUC 0.854 | Sensitivity 0.84
"""
...
ALL_FEATURES = [
    ...
    "FIBTEMMCF", "FIBTEMAUC",  "Hb",         "PLT",        "INR",
    "Albumin",   "Creatinine", "Bilirubin",  "Fibrinogen", "TLC",
    "Urea",      "MELD",       "CTP",        "AKI",        "PriorDecompensation",
...
@st.cache_resource(show_spinner="Training GBM model — please wait ...")
def train_gbm(uploaded_file):
    ...
    # 60 / 20 / 20 split
    X_temp, _X_test, y_temp, _y_test = train_test_split(
    ...
    # Oversample minority on training split only
    df_tr          = X_train.copy()
    ...
    # Tune threshold on validation set: sensitivity >= 80%
...
with st.sidebar:
    ...
    uploaded = st.file_uploader(
        "Upload NVCB_anonymised_dataset.xlsx",
        type=["xlsx"],
        help="The anonymised NVCB dataset in Excel format.",
    )
...
st.markdown(
    "*Institute of Liver and Biliary Diseases (ILBS) &middot; "
    "GBM Model &middot; AUC 0.854 &middot; Sensitivity 0.84*"
)
st.divider()

if uploaded is None:
    st.info("⬅️ Please upload the NVCB dataset in the sidebar to train the model.")
    st.stop()
...
# ── Prediction ─────────────────────────────────────────────────────────────────
if st.button("🔍 Predict NVCB Risk", type="primary", use_container_width=True):
    ...
    st.markdown(
        f"**Probability score:** `{prob:.3f}` &nbsp;|&nbsp; **Threshold:** `{threshold:.3f}`"
    )
    st.divider()
...
Developed at ILBS &middot; GBM Model &middot; AUC 0.854 &middot; Sensitivity 0.84 &middot; n = 6,992 patients
</small>
""", unsafe_allow_html=True)
